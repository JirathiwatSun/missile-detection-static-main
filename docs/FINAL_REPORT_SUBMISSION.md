...
See [PRESENTATION_TALK_TRACK_CHEATSHEET.md](./PRESENTATION_TALK_TRACK_CHEATSHEET.md) for what to show evaluators.
...
### 6. AI Model Performance & Tracking Accuracy (Runs Data)

To provide verifiable proof of the system's effectiveness, we have included the full training results from our latest run (`runs/detect/missile_yolo26_custom/`). These metrics demonstrate high-fidelity tracking capability in both daytime and high-noise environments.

#### **6.1. Training Summary (Epoch 100)**
| Metric | Value |
| :--- | :--- |
| **Precision** | 85.0% |
| **Recall** | 76.9% |
...
#### **6.2. Performance Visuals**

> [!NOTE]
> Training results automatically generated by the YOLOv8 engine.

![Training Results](../runs/detect/missile_yolo26_custom/results.png)
*Figure 1: Loss curves and accuracy metrics over 100 epochs.*

![Confusion Matrix](../runs/detect/missile_yolo26_custom/confusion_matrix_normalized.png)
*Figure 2: Normalized Confusion Matrix showing high classification accuracy.*

![Precision-Recall Curve](../runs/detect/missile_yolo26_custom/BoxPR_curve.png)
*Figure 3: Precision-Recall (PR) Curve illustrating the optimal sensitivity-specificity balance.*

#### **6.3. Interpretation**
The consistently high mAP@50 score (83.9%) confirms that the model is exceptionally reliable at detecting missiles even with significant atmospheric interference. The PR curve demonstrates that our precision remains stable across varying recall thresholds, making it suitable for high-stakes interception scenarios where zero false negatives are critical.
